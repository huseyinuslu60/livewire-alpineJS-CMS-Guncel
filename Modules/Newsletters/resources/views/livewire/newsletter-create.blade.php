        ...
        <div class="xl:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="mb-6">
                    ...
                    <div class="mb-8">
                        <div class="flex justify-between items-center mb-4">
                            <h4 class="text-md font-semibold text-gray-800">📧 Bülten Template Seçin</h4>
                        </div>
                        <div class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 gap-2">
                            @foreach($availableTemplates as $template)
                                <div class="template-card {{ $selectedTemplate == $template->id ? 'selected' : '' }}"
                                     wire:click="selectTemplate({{ $template->id }})"
                                     style="cursor: pointer; border: 2px solid {{ $selectedTemplate == $template->id ? '#3b82f6' : '#e5e7eb' }}; border-radius: 6px; padding: 6px; background: {{ $selectedTemplate == $template->id ? '#f0f9ff' : 'white' }}; transition: all 0.3s ease;">
                                    
                                    <!-- Template Preview -->
                                    <div class="template-preview mb-1" style="height: 40px; background: {{ $template->styles['background_color'] ?? '#f8fafc' }}; border-radius: 3px; padding: 4px; overflow: hidden; position: relative;">
                                        <!-- Header Preview -->
                                        <div style="background: {{ $template->styles['primary_color'] ?? '#3b82f6' }}; height: 10px; border-radius: 2px; margin-bottom: 3px; display: flex; align-items: center; padding: 0 3px;">
                                            <div style="width: 6px; height: 6px; background: white; border-radius: 1px; margin-right: 3px;"></div>
                                            <div style="width: 20px; height: 3px; background: rgba(255,255,255,0.7); border-radius: 1px;"></div>
                                        </div>
                                        
                                        <!-- Content Preview -->
                                        <div style="display: flex; gap: 3px;">
                                            <div style="width: 12px; height: 10px; background: #e5e7eb; border-radius: 1px;"></div>
                                            <div style="flex: 1;">
                                                <div style="width: 100%; height: 2px; background: #dc2626; border-radius: 1px; margin-bottom: 1px;"></div>
                                                <div style="width: 80%; height: 1px; background: #6b7280; border-radius: 1px; margin-bottom: 1px;"></div>
                                                <div style="width: 60%; height: 1px; background: #6b7280; border-radius: 1px;"></div>
                                            </div>
                                        </div>
                                        
                                        <!-- Footer Preview -->
                                        <div style="position: absolute; bottom: 1px; left: 3px; right: 3px; height: 4px; background: {{ $template->styles['secondary_color'] ?? '#f3f4f6' }}; border-radius: 1px;"></div>
                                    </div>
                                    
                                    <!-- Template Info -->
                                    <div>
                                        <h5 class="font-semibold text-gray-900 text-xs mb-1 line-clamp-1">{{ $template->name }}</h5>
                                        <p class="text-xs text-gray-600 mb-1 line-clamp-1">{{ $template->description }}</p>
                                        
                                        <!-- Template Colors -->
                                        <div class="flex gap-1 mb-1">
                                            <div style="width: 6px; height: 6px; background: {{ $template->styles['primary_color'] ?? '#3b82f6' }}; border-radius: 1px;"></div>
                                            <div style="width: 6px; height: 6px; background: {{ $template->styles['secondary_color'] ?? '#6b7280' }}; border-radius: 1px;"></div>
                                            <div style="width: 6px; height: 6px; background: {{ $template->styles['text_color'] ?? '#1f2937' }}; border-radius: 1px;"></div>
                                        </div>
                ...
                <div class="mt-6">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-3 max-h-[800px] overflow-y-auto">
                            ...
                            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                                ...
                                <!-- Email Content -->
                                <div class="p-4">
                                    <div class="newsletter-preview-content text-sm" style="transform: scale(0.8); transform-origin: top left; width: 125%;">
                                        {{-- sanitize edilmiş değişken --}}
                                        {!! \App\Support\Sanitizer::sanitizeHtml($mail_body) !!}
                                    </div>
                                </div>
                            </div>
                            </div>
